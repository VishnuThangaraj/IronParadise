...
`;
    sendMail(
      member.email,
      "IronParadise Membership Subscription Going to Expire",
      html
    );
...
// Subscription Amount UnPaid
const mailPayment = async (member) => {
  const plan = await MembershipPlan.findById(
    member.subscription.membershipPlan
  );
...
<body>
    <div class="container">
        ...
        <div class="content">
            ...
            <p>Your subscription plan of <strong>₹ ${
              member.subscription.planCost
            }</strong> has a unpaid pending amount of <strong>₹ ${
      member.subscription.pending
    }</strong>.</p><br/>
    <p>Plan Name : ${plan.name}</p><br/>
    <p>Plan Duration : ${plan.duration} Months</p><br/>
    <p>Subscription End Date : ${`${member.subscription.endDate}`.slice(
      0,
      16
    )}</p><br/>
            <p>To continue accessing all the benefits, please renew your subscription at your earliest convenience.</p>
    ...
    sendMail(member.email, "IronParadise Membership Payment Reminder", html);

    console.log("Mail Sent for Subscription Payment Reminder");
  } catch (err) {
    console.log("Failed to Send Mail for Subscription Payment Reminder");
  }
};

// Subscription Expiry General Notification
const mailGeneralExpiry = async (member) => {
  const plan = await MembershipPlan.findById(
    member.subscription.membershipPlan
  );
...
<body>
    <div class="container">
        ...
        <div class="content">
            ...
            <p>Your subscription plan of <strong>₹${
              member.subscription.planCost
            }</strong> going to end on <strong>${`${member.subscription.endDate}`.slice(
      0,
      16
    )}</strong>.</p><br/>
    <p>Plan Name : ${plan.name}</p><br/>
    <p>Plan Duration : ${plan.duration} Months</p><br/>
            <p>To continue accessing all the benefits in future, please renew your subscription at your earliest convenience.</p>
            <p>If you have any questions or need assistance with renewing, feel free to contact us!</p>
            <p>Sincerely,<br />Iron Paradise Admin</p>
        </div>
        <div class="footer">
    ...
    sendMail(
      member.email,
      "IronParadise Membership Subscription Reminder",
      html
    );

    console.log("Mail Sent for General Subscription Notification");
  } catch (err) {
    console.log("Failed to Send Mail for General Subscription Notification");
  }
};

// Subscription Mail
const subscriptionMail = async (req, res) => {
  const { notificationCategory } = req.body;
  try {
    ...
    members.forEach((member) => {
      if (notificationCategory === "everyone") {
        const subscriptionEndDate = moment(member.subscription.endDate).startOf(
          "day"
        );
        if (
          member.subscription.planCost === member.subscription.pending ||
          (member.subscription.pending != 0 &&
            member.subscription.planCost > member.subscription.pending)
        ) {
          mailPayment(member);
        } else if (subscriptionEndDate.isBefore(today)) {
          MailPast(member);
        } else if (
          ...
          mailGeneralExpiry(member);
        }
      } else {
        // Unpaid
        if (
          notificationCategory === "both" ||
          notificationCategory === "unpaid"
        ) {
          if (member.subscription.planCost === member.subscription.pending) {
            mailPayment(member);
          }
        }
